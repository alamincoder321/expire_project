...
class Account extends CI_Controller
{
    ...
    function getOtherIncomeExpense()
    {
        ...
        $result = $this->db->query("
            ...
            (
                select ifnull(sum(r.SaleReturn_ReturnAmount), 0)
                from tbl_salereturn r
                where r.Status = 'a'
        ...
        $transactions = $this->db->query("
            select * from(
                ...
                select 
                    'd' as sequence,
                    bt.transaction_id as id,
                    'Transfer In' as description,
                    bt.transfer_account_id as account_id,
                    bt.transaction_date,
                    bt.transaction_type,
                    bt.amount as deposit,
                    0 as withdraw,
                    ...
                    0.00 as balance
                from tbl_bank_transactions bt
                join tbl_bank_accounts ac on ac.account_id = bt.transfer_account_id
                where bt.status = 1
                ".(!empty($data->ledger) ? "and bt.transfer_account_id = '$data->accountId'" : "")."
                ...
                join tbl_bank_accounts ac on ac.account_id = bt.account_id
                where bt.status = 1
                ".(!empty($data->ledger) ? "and bt.account_id = '$data->accountId'" : "")."
                and bt.transaction_type = 'transfer'
                and bt.branch_id = " . $this->session->userdata('BRANCHid') . "
            ) as tbl
            where 1 = 1 $clauses
