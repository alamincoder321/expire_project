...
class Sales extends CI_Controller
{
    ...
    {
        $data = json_decode($this->input->raw_input_stream);
        $isService = isset($data->isService) ? $data->isService : '';
        $categoryId = isset($data->categoryId) ? $data->categoryId : '';
        $barcode = isset($data->name) ? trim((string)$data->name) : '';
        ...
        if ($isService !== '' && $isService !== null) {
            $clauses .= " and p.is_service = '$isService'";
        }

        $quantity = 1;
        $products = [];

        // Rongta flow only when barcode starts with 2100
        if (strpos($barcode, '2100') === 0) {
            $products = $this->db->query("
                ...
                where p.status = 'a'
                and ? like concat(p.Product_Code, '%')
                $clauses
                order by CHAR_LENGTH(p.Product_Code) desc, p.Product_SlNo desc
                limit 1
            ", [$barcode])->result();
        } else if (strpos($barcode, '2300') === 0) {
            $products = $this->db->query("
                select
                    p.*,
                    concat(p.Product_Name, ' - ', p.Product_Code) as display_text,
            ...
            // General simple flow: exact code match only
            $products = $this->db->query("
            select
                p.*,
                concat(p.Product_Name, ' - ', p.Product_Code) as display_text,
        ...
        // Strict prefix match from beginning only; reject partial middle matches.
        if (strpos($barcode, $productCode) !== 0) {
            echo json_encode([]);
            return;
        }
        ...
        if (strpos($barcode, '2100') === 0) {
            ...
            $weightPart = substr($barcode, strlen($productCode), 5);
            if (
                strlen($barcode) >= (strlen($productCode) + 5)
