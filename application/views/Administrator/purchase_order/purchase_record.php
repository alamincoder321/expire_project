		...
		<div class="col-md-12">
			<div class="table-responsive" id="reportContent">
				<table
					...
					<tbody>
						<template v-for="(purchase, sl) in purchases">
							<tr>
								<td>{{ sl + 1 }}</td>
				...
				<table
					...
					<tbody>
						<tr v-for="(purchase, sl) in purchases">
							<td>{{ sl + 1 }}</td>
							<td>{{ purchase.PurchaseMaster_InvoiceNo }}</td>
							<td>{{ purchase.PurchaseMaster_OrderDate }}</td>
							<td>{{ purchase.Supplier_Name }}</td>
	...
	new Vue({
		...
		methods: {
			checkReturnAndEdit(purchase) {
				location.replace('/purchase_order/' + purchase.PurchaseMaster_SlNo);
			},
			onChangeSearchType() {
				this.purchases = [];
			...
			getPurchaseRecord() {
				...
				let url = '/get_purchase_orders';
				if (this.recordType == 'with_details') {
					url = '/get_purchase_order_record';
				}
			...
			deletePurchase(purchaseId) {
				...
				axios.post('/delete_purchase_order', {
