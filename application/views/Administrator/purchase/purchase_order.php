					...
					<div class="row">
						...
						<div class="col-sm-6">
							<form v-on:submit.prevent="addToCart">
								<div class="form-group">
									<label class="col-xs-4 control-label no-padding-right"> Order </label>
									<div class="col-xs-8">
										<v-select v-bind:options="orders" v-model="selectedOrder" label="invoice_text" v-on:input="onChangeOrder"></v-select>
									</div>
								</div>
								...
								<div class="form-group">
									...
									<label class="col-xs-4 control-label" for="isFree" style="display: flex;align-items:center;cursor:pointer;">
										...
										<span style="margin: 0;margin-left: 6px;">Free Product</span>
									</label>
									<div class="col-xs-4">
	...
	new Vue({
		...
		methods: {
			...
			onChangeOrder() {
				...
				if (this.selectedOrder != null) {
					axios.post('/get_purchase_orders', {
						purchaseId: this.selectedOrder.PurchaseMaster_SlNo,
						status: 'p'
					}).then(res => {
			...
			savePurchase() {
				...
				this.purchase.orderId = this.selectedOrder ? this.selectedOrder.PurchaseMaster_SlNo : null;
				let data = {
			...
			getPurchaseOrder() {
				axios.post('/get_purchase_orders', {
					...
					this.orders = res.data.purchases;
				})
			}
		}
	})
