...
<div id="productLedger">
	...
	<div class="row" style="display:none;" v-bind:style="{display: showTable ? '' : 'none'}">
		...
		<div class="col-sm-6 text-right">
			<a href="" v-on:click.prevent="exportToExcel">
...
<script>
	...
	new Vue({
		...
		methods: {
			...
			exportToExcel() {
				let data = [];
				data.push(['Date', 'Description', 'Rate', 'In Quantity', 'Out Quantity', 'Stock']);
				data.push(['', 'Previous Stock', '', '', '', this.previousStock.toFixed(2)]);
				this.ledger.forEach(row => {
					data.push([row.date, row.description, row.rate.toFixed(2), row.in_quantity.toFixed(2), row.out_quantity.toFixed(2), row.stock.toFixed(2)]);
				});

				const worksheet = XLSX.utils.aoa_to_sheet(data);
				const workbook = XLSX.utils.book_new();
				XLSX.utils.book_append_sheet(workbook, worksheet, "Product Ledger");
