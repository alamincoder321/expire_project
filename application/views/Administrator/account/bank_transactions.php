                ...
                <div class="row">
                    <div class="col-md-6 col-md-offset-1">
                        <form action="" class="form-horizontal" @submit.prevent="saveTransaction">
                            ...
                            <div class="form-group" style="display: none;" :style="{display: transaction.transaction_type == 'transfer' ? '' : 'none'}" v-if="transaction.transaction_type == 'transfer'">
                                <label for="" class="control-label col-md-4">Transfer Account</label>
                                <div class="col-md-8">
                                    <v-select v-bind:options="filteredAccounts" v-model="selectedTransferAccount" label="display_text" placeholder="Select account" @input="getBankBalance"></v-select>
                                </div>
                            </div>
                            ...
                            <datatable :columns="columns" :data="transactions" :filter-by="filter">
                                <template scope="{ row }">
                                    <tr v-bind:class="[row.transaction_type == 'deposit' ? 'transaction-deposit' : 'transaction-withdraw']">
                                        ...
                                        <td>{{ row.bank_name }}</td>
                                        <td>
                                            <span v-if="row.transaction_type == 'transfer'">
                                                {{ row.transaction_type }} (Bank: {{row.transfer_account_number}} - {{row.transfer_account_name}})
                                            </span>
                                            <span v-else>{{ row.transaction_type }}</span>
                                        </td>
                                        <td>{{ row.note }}</td>
                                        <td>{{ row.amount }}</td>
                                        <td>{{ row.saved_by }}</td>
        ...
        methods: {
            ...
            saveTransaction() {
                ...
                if(this.transaction.transaction_type == 'transfer'){
                    if(this.selectedTransferAccount == null){
                        alert('Select a transfer account');
                        return;
                    }
                    this.transaction.transfer_account_id = this.selectedTransferAccount.account_id;
                }
