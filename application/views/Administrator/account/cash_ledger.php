		...
		methods:{
			...
			excelExport(){
				let onlyData = [{
					...
					'Cash In': '',
					'Cash Out': '',
					'Balance': parseFloat(this.previousBalance).toFixed(2)
				}];
				...
				this.ledger.forEach(row => {
					onlyData.push({
						...
						'Cash In': Number(row.in_amount),
						'Cash Out': Number(row.out_amount),
				...
				XLSX.writeFile(workbook, "CashLedgerReport.xlsx");
			},
			async print(){
				let reportContent = `
					<div class="container">
