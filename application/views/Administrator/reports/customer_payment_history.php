	...
	new Vue({
		...
		methods: {
			...
			exportToExcel() {
				...
				let ws_data = [];
				let headerRow = ['Transaction Id', 'Date', 'Customer', 'Transaction Type', 'Payment by', 'Description', 'Amount'];
				ws_data.push(headerRow);

				this.payments.forEach(payment => {
					let row = [
						payment.CPayment_invoice,
						payment.CPayment_date,
						`${payment.Customer_Code} - ${payment.Customer_Name}`,
						payment.transaction_type,
						payment.payment_by,
						payment.CPayment_notes,
						payment.CPayment_amount
					];
					ws_data.push(row);
				});

				let ws = XLSX.utils.aoa_to_sheet(ws_data);
				XLSX.utils.book_append_sheet(wb, ws, "Customer Payment History");
				XLSX.writeFile(wb, "Customer_Payment_History.xlsx");
			}
		}
	})
</script>